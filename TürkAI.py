...
import urllib.parse
import re 
import json # TDK JSON verileri için gerekli
...
st.markdown("""
    ...
    .sonuc-metni { padding: 25px; border-radius: 12px; border: 1px solid rgba(128, 128, 128, 0.2); line-height: 1.7; background: rgba(128, 128, 128, 0.02); font-size: 1.05rem; white-space: pre-wrap; }
    </style>
...
# --- 🔧 AKILLI VERİ TEMİZLEME (Yeni Cerrah Fonksiyonu) ---

def icerik_temizle(soup, url):
    """Sitedeki menü, footer, link ve referansları yok eder."""
    # 1. Gereksiz HTML etiketlerini imha et
    for gereksiz in soup(['nav', 'footer', 'header', 'aside', 'script', 'style', 'noscript', 'form', 'label']):
        gereksiz.decompose()
    
    # 2. Wikipedia ve Bilgiustam için özel çöp temizliği
    for div in soup.find_all(['div', 'span'], class_=re.compile(r'menu|footer|sidebar|comment|related|nav|social|mw-editsection|reflist|toc')):
        div.decompose()

    metin = soup.get_text(separator='\n')
    
    # 3. Regex ile Link ve Referans Temizliği
    metin = re.sub(r'\[\d+\]', '', metin) # [1], [2] gibi kaynak linklerini siler
    metin = re.sub(r'\[değiştir\s*\|\s*kaynağı değiştir\]', '', metin) # Wikipedia butonlarını siler
    metin = re.sub(r'http[s]?://\S+', '', metin) # Çıplak URL'leri siler
    
    # 4. Satır Süzgeci (Kısa buton yazılarını eler)
    satirlar = [s.strip() for s in metin.splitlines() if len(s.strip()) > 45] 
    return "\n\n".join(satirlar)

def tdk_json_coz(ham_metin):
    """TDK'nın kod yığınını temizleyip sadece anlamları alır."""
    try:
        veri = json.loads(ham_metin)
        anlamlar = []
        for madde in veri:
            if "anlamlarListe" in madde:
                for a in madde["anlamlarListe"]:
                    anlamlar.append(f"• {a['anlam']}")
            if "atasozu" in madde:
                for at in madde["atasozu"]:
                    anlamlar.append(f"ℹ️ Deyim/Atasözü: {at['madde']}")
        return "\n".join(anlamlar) if anlamlar else "Tanım bulunamadı."
    except: return "Veri işleme hatası."
...
def pdf_olustur(baslik, icerik):
    try:
        ...
        pdf.ln(5)
        pdf.set_font("Arial", '', 11); pdf.multi_cell(0, 6, tr_fix(icerik))
...
def site_tara_brave_style(url, sorgu, site_adi):
    try:
        ...
        if "sozluk.gov.tr" in url:
            return (site_adi, tdk_json_coz(response.text))
            
        soup = BeautifulSoup(response.content, 'html.parser')
        final = icerik_temizle(soup, url)
            
        return (site_adi, final) if len(final) > 30 else (site_adi, None)
    except: return (site_adi, None)

# --- (Giriş, Kayıt ve Görsel OCR bölümleri hiç değişmeden korunmuştur) ---

# ... [Orijinal Giriş/Kayıt ve OCR Kodlarınız Burada Devam Eder] ...

# --- 🚀 ANA PANEL (Düzenlenmiş Sorgu Akışı) ---
if not st.session_state.user:
    # Giriş ekranı (Yukarıdaki orijinal kodunuz)
...
with st.sidebar:
    ...
    if calc:
        if re.search(r'[a-zA-Z]', calc): st.error("Lütfen sadece işlem yazınız.")
        else:
    ...
    if yuklenen_dosya:
        ...
        if st.button("📄 Yazıları Çıkar ve PDF Yap", use_container_width=True):
            with st.spinner('Okunuyor...'):
                ...
                if text.strip():
                    st.text_area("Okunan:", text, height=200)
                    pdf_data = pdf_olustur("Görsel Analiz", text)
                    st.download_button("📥 PDF İndir", pdf_data, "TurkAI_OCR.pdf")
    ...
    if sorgu:
        ...
        with st.spinner("Veriler ayıklanıyor..."):
            if m_secim == "Sözlük (TDK)":
                ...
                urls = [f"https://tr.wikipedia.org/wiki/{q_enc}", f"https://www.bilgiustam.com/?s={q_enc}"]
                st.session_state.tum_kaynaklar = [site_tara_brave_style(u, sorgu, "Analiz") for u in urls if site_tara_brave_style(u, sorgu, "Analiz")[1]]
...
if st.session_state.bilgi:
    ...
    if pdf: st.download_button("📥 PDF İndir", pdf, f"{st.session_state.konu}.pdf", use_container_width=True)
