...
st.markdown("""
    ...
    /* Sadece Kullanıcı Mesaj Balonu (Sağda) */
    .user-msg {
    ...
    /* TürkAI Cevap Alanı (Normal Yazı Tipi) */
    .ai-cevap-alani {
    ...
    .giris-kutu { border: 2px solid #cc0000; padding: 25px; border-radius: 15px; text-align: center; }
    </style>
""", unsafe_allow_html=True)

# --- 💾 VERİTABANI ---
def db_baslat():
    conn = sqlite3.connect('turkai_v126.db', check_same_thread=False)
    c = conn.cursor()
...
if not st.session_state.user:
    ...
    with col2:
        st.markdown("<div class='giris-kutu'><h1>🇹🇷 TürkAI</h1></div>", unsafe_allow_html=True)
...
if sorgu:
    ...
    if m_secim == "V3 (Hesap Makinesi)":
        try:
            ...
            c.execute("INSERT INTO aramalar (kullanici, konu, icerik, tarih, motor) VALUES (?,?,?,?,?)", (st.session_state.user, "Matematik", st.session_state.bilgi, str(datetime.datetime.now()), "V3"))
            conn.commit(); st.rerun()
        except: st.session_state.bilgi = "İşlem yapılamadı."
        ...
        try:
            ...
            if r['query']['search']:
                ...
                c.execute("INSERT INTO aramalar (kullanici, konu, icerik, tarih, motor) VALUES (?,?,?,?,?)", (st.session_state.user, head, txt, str(datetime.datetime.now()), "V1"))
                conn.commit(); st.rerun()
            else: st.session_state.bilgi = "Sonuç bulunamadı."
        except: st.session_state.bilgi = "Veriye ulaşılamadı."
