...
st.markdown("""
    ...
    .giris-kapsayici { border: 1px solid rgba(204, 0, 0, 0.3); border-radius: 12px; padding: 30px; background: rgba(255,255,255,0.05); }
...
def pdf_olustur(baslik, icerik):
    try:
        ...
        pdf.set_font("Arial", 'B', 14); pdf.cell(0, 10, tr_fix(baslik), ln=True)
        pdf.set_font("Arial", '', 10); pdf.multi_cell(0, 5, tr_fix(icerik))
        return pdf.output(dest='S').encode('latin-1', 'ignore')
    except: return None

def akiskan_yazi(metin):
    """Yazıyı tane tane ekrana basar"""
    placeholder = st.empty()
    full_text = ""
    for char in metin:
        full_text += char
        placeholder.markdown(f"<div style='color: #333; line-height: 1.6;'>{full_text}▌</div>", unsafe_allow_html=True)
        time.sleep(0.001) # Çok hızlı ama akıcı
    placeholder.markdown(f"<div style='color: #333; line-height: 1.6;'>{full_text}</div>", unsafe_allow_html=True)
...
if not st.session_state.user:
    ...
    with col2:
        st.markdown("<div class='giris-kapsayici'><h2 style='text-align:center;'>🇹🇷 TürkAI V1</h2>", unsafe_allow_html=True)
        tab_in, tab_up, tab_m = st.tabs(["🔑 Giriş", "📝 Kayıt Ol", "👤 Misafir"])
        with tab_in:
        ...
        with tab_m:
            if st.button("Yetkisiz Giriş", use_container_width=True): st.session_state.user = "Misafir"; st.rerun()
...
if st.session_state.bilgi:
    ...
    # Yazının Tane Tane Akması
    st.markdown("<div style='background: #fdfdfd; padding: 20px; border-left: 5px solid #cc0000;'>", unsafe_allow_html=True)
