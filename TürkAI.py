...
st.markdown("""
    ...
    .kaynak-atfi {
        font-size: 0.85rem; color: #888; font-style: italic; margin-bottom: 20px;
    }

    .mat-not {
        background-color: #fff3f3; color: #cc0000; padding: 10px;
        border-radius: 10px; border: 1px dashed #cc0000; margin-top: 10px;
    }
    ...
    /* PDF Yazdırma Ayarı */
    @media print {
        .stSidebar, .stChatInput, .stButton { display: none !important; }
        .user-msg { background: #cc0000 !important; color: white !important; }
    }
    </style>
...
if not st.session_state.user:
    ...
    with col2:
        st.markdown("<div style='text-align:center; border:2px solid #cc0000; border-radius:20px; padding:20px;'><h1>🇹🇷 TürkAI</h1></div>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if m_secim == "V3 (Matematik)":
        st.markdown("<div class='mat-not'>⚠️ <b>NOT:</b> İşlemlerde çarpı (x) yerine yıldız (<b>*</b>) sembolü kullanınız kanka.</div>", unsafe_allow_html=True)
...
if sorgu:
    ...
    if m_secim == "V1 (Wikipedia)":
        ...
        try:
            ...
            if 'extract' in r_wiki: bilgi = r_wiki['extract']
            else:
                ...
                bilgi = snippet.get_text() if snippet else "Kaynak bulunamadı."
            st.session_state.bilgi, st.session_state.konu = bilgi, sorgu.title()
        except: st.session_state.bilgi = "Sistem meşgul."
...
if st.session_state.bilgi:
    ...
    # ATIFLAR
    if "V1" in st.session_state.aktif_motor:
        st.markdown("<div class='kaynak-atfi'>📍 Kaynak: Wikipedia Veritabanı</div>", unsafe_allow_html=True)
    elif "V2" in st.session_state.aktif_motor:
        st.markdown("<div class='kaynak-atfi'>📍 Kaynak: Global Network & Hibrit Motor</div>", unsafe_allow_html=True)

    # PDF BUTONU (Yazdırma Tetikleyici)
    if st.button("📄 Analizi PDF Olarak Kaydet"):
