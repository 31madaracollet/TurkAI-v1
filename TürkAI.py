...
def yabanci_karakter_temizle(metin):
    if not metin: return ""
    # Sadece Türkçe karakterler ve temel noktalama işaretlerini koru
    return re.sub(r'[^a-zA-Z0-9çğıöşüÇĞİÖŞÜ\s\.\,\:\;\-\!\(\)]', '', metin)

def wikipedia_temizle(metin):
    # Köşeli parantez içindeki referans numaralarını temizle
    metin = re.sub(r'\[\d+\]', '', metin)
    
    # Menü ve navigasyon öğelerini temizle
    silinecekler = [
        "İçeriğe atla", 
...
def tdk_temizle(metin):
    # TDK'daki gereksiz link benzeri ifadeleri temizle
    metin = re.sub(r'/[^ ]*', '', metin)
    metin = re.sub(r'\([^)]*\)', '', metin)  # Parantez içlerini temizle
    return metin.replace("null", "").replace("GTS", "").strip()

def icerik_dogrula(metin, aranan_kelime):
    """Alınan içeriğin aranan kelimeyi içerip içermediğini kontrol et"""
    if not metin or not aranan_kelime:
        return False
    
    # Aranan kelimeyi küçük harfe çevir ve kelime sınırlarıyla ara
    aranan = aranan_kelime.lower()
    metin_kucuk = metin.lower()
    
    # Basit kelime eşleşmesi
    if aranan in metin_kucuk:
        return True
    
    # Alternatif yazımlar (TDK için)
    alternatifler = [
        aranan,
        aranan.replace('i', 'ı'),
        aranan.replace('ı', 'i'),
    ...
    for alt in set(alternatifler):
        if alt in metin_kucuk:
            return True
...
def pdf_olustur(baslik, icerik):
    try:
        ...
        pdf.cell(0, 10, tr_fix(baslik), ln=True, align='C')
...
def site_tara_brave_style(url, sorgu, site_adi):
    try:
        ...
        for j in soup(['script', 'style', 'nav', 'footer', 'header', 'aside']): 
            j.decompose()
        
        # Siteye özel içerik çekme
        if "wikipedia" in url:
            content = soup.find(id="mw-content-text")
            final = wikipedia_temizle(content.get_text() if content else soup.get_text())
        elif "sozluk.gov.tr" in url:
            # TDK için özel işlem
            content = soup.find('div', {'id': 'anasayfa'})
            if not content:
                content = soup.find('div', {'class': 'gts-icerik'})
            final = tdk_temizle(content.get_text() if content else soup.get_text())
        elif "biyografi.info" in url:
            content = soup.find('div', {'class': 'icerik'})
            final = content.get_text() if content else soup.get_text()
        elif "islamansiklopedisi" in url:
            content = soup.find('div', {'class': 'search-result'})
            final = content.get_text() if content else soup.get_text()
        elif "worldhistory" in url:
        ...
        if not icerik_dogrula(final, sorgu):
...
with st.sidebar:
    ...
    if calc:
        if re.search(r'[a-zA-Z]', calc):
            ...
            try: 
                st.success(f"Sonuç: {eval(re.sub(r'[^0-9+\-*/(). ]', '', calc))}")
            except: 
                st.error("Hatalı işlem!")
    st.divider()
        ...
        if m_secim == "V3 (Matematik)" and re.search(r'[a-zA-Z]', sorgu):
            ...
            with st.container():
                ...
                if m_secim == "🤔 Derin Düşünen":
                    # Güncellenmiş kaynak listesi - ansiklopedik, sözlük ve biyografik ağırlıklı
                    siteler = [
                    ...
                    for i, url in enumerate(siteler):
                        site_adi = urllib.parse.urlparse(url).netloc
                        status.info(f"🔍 {site_adi} taranıyor... ({i+1}/{len(siteler)})")
                        p_bar.progress((i+1)/len(siteler))
                        
                        res = site_tara_brave_style(url, sorgu, f"{site_adi}")
                        
                        # Sadece içerik doğrulamadan geçenleri ekle
                        if res[1]:  
                            bulunanlar.append(res)
                            status.success(f"✅ {site_adi}'den bilgi bulundu!")
                        else:
                            status.warning(f"⚠️ {site_adi}'de uygun bilgi bulunamadı.")
                        
                        time.sleep(0.5)  # Saygın tarama için kısa bekleme
                    
                    st.session_state.tum_kaynaklar = bulunanlar
                    
                    if bulunanlar:
                        status.success(f"✅ Toplam {len(bulunanlar)} kaynakta bilgi bulundu!")
                    else:
                        status.error("❌ Hiçbir kaynakta bilgi bulunamadı!")
                    ...
                    try:
                        ...
                        st.session_state.tum_kaynaklar = []
                        
                else: # V1
                    p_bar.progress(50)
                    res = site_tara_brave_style(f"https://tr.wikipedia.org/wiki/{q_enc}", sorgu, "V1 (Wikipedia)")
                    st.session_state.tum_kaynaklar = [res] if res[1] else []
                    p_bar.progress(100)
                ...
                if st.session_state.tum_kaynaklar:
                    ...
                    st.session_state.bilgi = "Üzgünüm, bu konuda yeterli veri bulunamadı."
            ...
            with c1:
                ...
                if pdf: 
                    st.download_button(
                        "📥 PDF Olarak İndir", 
                        pdf, 
                        f"TurkAI_{st.session_state.konu}.pdf", 
                        use_container_width=True
                    )
